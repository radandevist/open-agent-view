...
| New task | `tab`, verify the harness/default-model and full launch-workspace title, type text, `ctrl+j`, type another line, then `esc` to discard. Exercise `/workspace` with a full remembered path and `/yolo` confirmation/cancellation without losing the draft. Repeat and `enter`; the fixture-mode refusal is expected and must not repeat the prompt. Also verify a printable key from the list seeds the composer. |
...
| Task commands | `/help`, `enter` opens the full contextual help panel rather than a clipped footer. Verify `/harness` opens the picker; use `/harness NAME`, `/model NAME`, `/model default`, `/workspace`, `/workspace /absolute/path`, `/yolo`, `/setup NAME`, and `/completed show|hide`, verifying the composer/header after each. `/provider` remains an alias and `/filter TEXT` remains an explicit alternative to `ctrl+f`. |
